...
bit adjacentObjects(int gridSize, Object object1, Object object2) {
...
for (int i = 0; i < ARR_BND; i++) {
  Cell cell1 = cells1[i];
  if (cell1 != null) {
    Position[ARR_BND] neighbors = adjPositions(gridSize, cells1[i].position);
    for (int j = 0; j < ARR_BND; j++) {
      Cell cell2 = cells2[j];
      if (cell2 != null) {
...
Object closest(Scene scene, int gridSize, Object object, char[STR_BND] type) {
Object[ARR_BND] objects = flattenSceneObjects(scene.objects);
int minDist = 2 * gridSize + 1;
Object closestObj;
for (int i = 0; i < ARR_BND; i++) {
  Object object2 = objects[i];
...
Object[ARR_BND] initObjectPositions(int gridSize, fun constructor, fun filterFun) {
int maxGridIndex = gridSize * gridSize - 1;
Object[ARR_BND] objects;
int objectIndex = 0;
for (int i = 0; i < maxGridIndex; i++) {
  Position pos = new Position(x=i%gridSize, y=i/gridSize);
  if (filterFun(pos)) {
