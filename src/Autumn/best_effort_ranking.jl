...
function get_fitness(p, e, a)::Float64
  return (distance_coefficient * get_distance(p, e) +
  size_coefficient * get_size(a))
end
...
function get_distance(p, e)::Float64
  overall_distance = 0
...
#return number of autumn nodes in the program p
function get_size(a)::Float64
  return sizeA(a)
end

function euclidean_distance(x1, y1, x2, y2)::Float64
