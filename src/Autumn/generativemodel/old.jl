function generate_global_automaton_sketch(small_event_vector_dict, augmented_positive_times, grouped_ranges, global_var_dict, global_var_id, global_var_value)
  ...
  ## run sketch on file 
  command = "$(sketch_directory)sketch --bnd-unroll-amnt $(length(event_string) + 2) --fe-output-code automata_sketch.sk"
  sketch_output = readchomp(eval(Meta.parse("`$(command)`")))

  ## modify intAsChar and add main function to produced cpp file 
  f = open("automata_sketch.cpp")
