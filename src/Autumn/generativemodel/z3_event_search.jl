...
function event_search_z3_concat(event_vector_dict, observed_data_dict) 
  ...
  for event in keys(event_vector_dict)
    ...
    # check that event is global or object-specific with the right type
    if event_values isa AbstractArray 
      event_vector_dict = Dict(map(id -> id => map(t -> event_values[t], filter(pos -> !(pos in ambig_positions[id]), collect(1:length(event_values)))), object_ids))
      atom_dict[event] = event_vector      
    elseif Set(collect(keys(event_values))) == Set(object_ids)
      event_vector_dict = Dict(map(id -> id => map(t -> event_values[id][t], filter(pos -> !(pos in ambig_positions[id]), collect(1:length(event_values[id])))), object_ids))
      atom_dict[event] = event_vector
    end
  end

  # get integer values of observed bit vectors
  observed_bv_vals_dict = Dict(map(id -> id => bitarr_to_int(observed_bv_dict[id]), observed_bv_dict)) # object_id => int value of observed bit vector
  bitsize_dict = Dict(map(id -> id => length(observed_bv_dict[id]), observed_bv_dict)) # object_id => length of observed bit vector

  # construct Z3 problem 
  ctx = Context()

  z3_atoms = [constant(ctx, "atoms$(i)", array_sort(ctx, int_sort(ctx), bv_sort(ctx, length(observed_bv_vals_dict[i])))) for i in 1:length(object_ids)]
  for object_id in 1:length(object_ids)
    # list of bit vector int values corresponding to given object_id for each event (sorted)
    atoms = map(event -> bitarr_to_int(atom_dict[event][object_id]), sort(collect(keys(atom_dict))))
    i = 0 # Z3 arrays are 0-indexed? or 1-indexed? using 0-indexing currently
    for elem in atoms
      z3_atoms[object_id] = Store(z3_atoms[object_id], i, elem)
      i = i + 1
    end      
  end
