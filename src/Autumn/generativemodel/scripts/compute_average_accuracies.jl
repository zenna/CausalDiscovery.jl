...
results_folders = readdir("APRIL_FINAL_RESULTS")
curr_result_folder = filter(x -> !occursin(".txt", x) && !occursin(".DS", x), results_folders)[end]

time_averages_file_name = "APRIL_FINAL_RESULTS/$(curr_result_folder)/average_times.txt"

model_folders = filter(x -> !occursin(".DS", x) && !occursin(".txt", x), readdir("APRIL_FINAL_RESULTS/$(curr_result_folder)"))
...
for model_folder in model_folders
  ...
  for alg_folder in ["heuristic", "sketch_multi", "sketch_single"]
    if alg_folder in readdir("APRIL_FINAL_RESULTS/$(curr_result_folder)/$(model_folder)")
      # # @show alg_folder
      times_file_name = "APRIL_FINAL_RESULTS/$(curr_result_folder)/$(model_folder)/$(alg_folder)/times.txt"
      try 
