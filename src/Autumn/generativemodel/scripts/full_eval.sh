...
do
  ...
  do
    ...
    do
      ...
      nohup /scratch/riadas/julia-1.5.4/bin/julia --project=. src/Autumn/generativemodel/scripts/full_eval.jl $model_name $algorithm $curr_date $i > bg_outs/$model_name_$algorithm.out 2>&1 &
      echo $! >> bg_pids.txt
      sleep 60
    done  
  done
