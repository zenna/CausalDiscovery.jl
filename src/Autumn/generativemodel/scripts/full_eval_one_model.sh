...
do
  ...
  julia --project=. src/Autumn/generativemodel/scripts/full_eval.jl $1 heuristic $2 $i > bg_outs/$1_heuristic.out 2>&1

  echo "model_name: $1, algorithm: sketch_multi, repeat: $i "
  julia --project=. src/Autumn/generativemodel/scripts/full_eval.jl $1 sketch_multi $2 $i > bg_outs/$1_sketch_multi.out 2>&1 &

  echo "model_name: $1, algorithm: sketch_single, repeat: $i "
  julia --project=. src/Autumn/generativemodel/scripts/full_eval.jl $1 sketch_single $2 $i > bg_outs/$1_sketch_single.out 2>&1 &
done
