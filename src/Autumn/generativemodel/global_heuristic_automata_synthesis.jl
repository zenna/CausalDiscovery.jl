...
function generate_new_state_GLOBAL(co_occurring_event, times_dict, event_vector_dict, object_trajectory, init_global_var_dict, state_update_times_dict, object_decomposition, type_id, user_events, desired_per_matrix_solution_count, user_event_simplified=false, interval_painting_param=false, transition_param=false, ordered_update_functions=[], transition_distinct=1, transition_same=1, transition_threshold=1, num_transition_decisions=15; bias_first_events=false) 
  ...
  if length(object_ids_with_type) != 1 
    for e in keys(small_event_vector_dict)
      if !isnothing(match(r"obj\d+", e)) # !occursin("clicked", e) && !occursin("intersects", e)
        delete!(small_event_vector_dict, e)
      end
    end
  end
...
function generate_stateless_on_clauses(run_id, update_functions_dict, matrix, filtered_matrix, anonymized_filtered_matrix, global_object_decomposition, user_events, state_update_on_clauses, global_var_dict, global_event_vector_dict, redundant_events_set, z3_option="full", time_based=false, z3_timeout=0, sketch_timeout=0)
  ...
  for type_id in type_ids
    ...
    addObj_count = 0
    if (length(unique_addObj_rules) > 1) && (length(unique(collect(values(addObj_times_dict)))) == 1) || (length(unique_addObj_rules) == 1 && occursin("randomPositions", unique_addObj_rules[1]))
...
function generate_new_object_specific_state_GLOBAL(global_events, co_occurring_event, update_functions, times_dict, event_vector_dict, type_id, object_decomposition, init_state_update_times, global_var_dict, transition_param=false, z3_option="full", time_based=false, z3_timeout=0, sketch_timeout=0, transition_distinct=1, transition_same=1, transition_threshold=1, num_transition_decisions=15)
  ...
  for transition_decision_string in transition_decision_strings 
    ...
    while length(grouped_ranges) > 0 && (iters < 50)
      ...
      if events_in_range == [] # if no global events are found, try object-specific events 
        ...
        events_in_range = find_state_update_events_object_specific(global_events, small_event_vector_dict, augmented_positive_times_dict, grouped_range, object_ids, object_mapping, curr_state_value)
      end
      @show events_in_range
      ...
      if length(events_in_range) > 0 # only handling perfect matches currently (UPDATE: <- now generalized)
        ...
        @show state_update_times 
        false_positive_events = find_state_update_events_object_specific_false_positives(global_events, small_event_vector_dict, augmented_positive_times_dict, grouped_range, object_ids, object_mapping, curr_state_value)      
      ...
      if !global_events 
        ...
        for object_id in object_ids
          if length(augmented_positive_times_dict[object_id]) != 0 
            ...
            # @show state_update_times
            no_state_updates = length(unique(collect(Base.values(state_update_times[field_id])))) == 1
