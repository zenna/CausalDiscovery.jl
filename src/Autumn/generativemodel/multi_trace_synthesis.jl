  ...
  for i in 2:length(object_decomposition_list)
    ...
    for id in curr_ids 
      if !isnothing(curr_object_mapping[id][1]) # object exists at start
        if !(nothing in prev_object_mapping[id]) # CASE 1: object always exists in this prev and curr object_mapping 
          push!(object_mapping[id], curr_object_mapping[id]...)
        else # CASE 2: object is removed in prev object_mapping
