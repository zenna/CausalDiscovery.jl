...
function singletimestepsolution_matrix(observations, old_user_events, grid_size; singlecell=false, pedro=false, upd_func_space=1, multiple_traces=false)
  ...
  for object_type in object_types 
    ...
    if unique(unique_position_counts) == [1] && (object_type.custom_fields == []) # every id occupies exactly one position and doesn't undergo a color change
  ...
  for time in 2:num_times
    ...
    for object_id in 1:num_objects
      update_functions, unformatted_update_functions, prev_used_rules, prev_abstract_positions = synthesize_update_functions(object_id, time, object_decomposition, user_events, prev_used_rules, prev_abstract_positions, stationary_types, grid_size, max_iters, upd_func_space, pedro=pedro, stop_times=stop_times)
...
function synthesize_update_functions(object_id, time, object_decomposition, user_events, prev_used_rules, prev_abstract_positions, stationary_types, grid_size=16, max_iters=11, upd_func_space=1; pedro=false, stop_times=[])
  ...
  elseif isnothing(next_object)
    ...
    while (iters < max_iters) # && length(solutions) < 1
      ...
      if is_stationary 
        push!(unformatted_solutions, "(= objX objX)")
        contained_in_list = isnothing(object_mapping[object_id][1]) || (count(id -> (filter(x -> !isnothing(x), object_mapping[id]))[1].type.id == object_mapping[object_id][1].type.id, collect(keys(object_mapping))) > 1)
        if contained_in_list 
          push!(solutions, "(= addedObjType$(type_id)List (updateObj addedObjType$(type_id)List (--> obj (prev obj)) (--> obj (== (.. obj id) $(object_id)))))")
        else
          push!(solutions, "(= obj$(object_id) (prev obj$(object_id)))")
        end
        break
      end
...
function parse_and_map_objects(observations, gridsize=16; singlecell=false, pedro=false)
  ...
  if !pedro 
    ...
    if !overlapping_cells 
      ...
      colors = vcat(map(t -> t.custom_fields == [] ? [t.color] : t.custom_fields[1][3] , square_types)...)
    ...
    for object in curr_objects
      ...
      if length(next_objects) != 0
        if minimum(distances) == 0
          ...
          # check if object type has color field; if so, cannot have next object with different color at the moment (current Autumn benchmark suite)
          if object_type.custom_fields != []
            ...
            closest_ids = map(obj -> obj.id, filter(o -> distance(object.position, o.position) == minimum(distances) && minimum(distances) < 5 * unitSize, next_objects_of_same_color))
            push!(closest_objects, (object.id, closest_ids))
          else
            push!(closest_objects, (object.id, map(obj -> obj.id, filter(o -> distance(object.position, o.position) == minimum(distances) && minimum(distances) < 5 * unitSize, next_objects))))
          end
        end 
      end
    end
...
function singletimestepsolution_program_given_matrix_NEW(matrix, object_decomposition, global_var_dict, state_update_on_clauses, event_string, grid_size=16; stop_times=[])  
  ...
  for row in 1:size(matrix_copy)[1]
    for col in 1:size(matrix_copy)[2]
      matrix_copy[row, col] = filter(x -> !occursin(".. click", x) && !occursin("uniformChoice", x) && !occursin("randomPositions", x) && !occursin("Random", x) && !occursin("firstWithDefault", x), matrix_copy[row, col])
    end
  end
...
function generate_on_clauses(matrix, unformatted_matrix, object_decomposition, user_events, global_event_vector_dict, redundant_events_set, grid_size=16, desired_solution_count=1, desired_per_matrix_solution_count=1, interval_painting_param=false, z3_option="full", time_based=true, z3_timeout=0, sketch_timeout=0)
...
function filter_update_function_matrix_multiple(matrix, object_decomposition; multiple = true, base = 2)
  ...
  for perm in 0:min(64, num_permutations)
    # @show perm
      ...
      if length(object_type.custom_fields) == 0 # type has no color field
        ...
        for time in 1:size(matrix)[2]
          ...
          if length(update_functions) > 1 
            ...
            if bit_value >= length(global_sorted_update_functions)
              top_function = global_sorted_update_functions[end]
            else
              top_function = global_sorted_update_functions[bit_value + 1]
            end

            if !(top_function in sorted_local_update_functions)
              top_function = sorted_local_update_functions[1]
            end
...
function construct_filtered_matrices(matrix, unformatted_matrix, object_decomposition, user_events, random=false, base=2)
  filtered_matrices = []
  ...
  if !random 
    ...
    for m in pre_filtered_matrices
      push!(filtered_matrices, filter_update_function_matrix_multiple(m, object_decomposition, multiple=true)...)
    end
    ...
    if length(filtered_matrices) > 6 
      filtered_matrices = filtered_matrices[1:6]
    end
...
function pre_filter_with_direction_biases(matrix, user_events, object_decomposition)
  ...
  for object_type in object_types 
    type_id = object_type.id
    new_matrix = deepcopy(matrix)
    object_ids_with_type = filter(id -> filter(o -> !isnothing(o), object_mapping[id])[1].type.id == type_id, collect(keys(object_mapping)))
...
function generate_event(run_id, anonymized_update_rule, distinct_update_rules, object_id, object_ids, matrix, filtered_matrix, object_decomposition, user_events, state_update_on_clauses, global_var_dict, event_vector_dict, grid_size, redundant_events_set, min_events=1, max_iters=400, z3_option = "none", time_based=true, z3_timeout=0, sketch_timeout=0, symmetry=false; stop_times=[])
  ...
  found_events = []
  final_event_globals = []
  choices = gen_event_bool(object_decomposition, "x", type_id, anonymized_update_rule, filter(e -> e != "", unique(user_events)), global_var_dict, time_based, symmetry)
  ...
  while true
    ...
    elseif z3_option in ["full", "partial"]
      ...
      for event in collect(keys(z3_event_vector_dict))
        ...
        if (z3_option == "partial") && (occursin("(in (Position", event) || occursin("&", event) && (event != "(& clicked (isFree click))"))
          delete!(z3_event_vector_dict, event) # only use &-based events (other than isFree click) during "full" z3 option
        end
      end
...
function z3_event_search_partial(observed_data_dict, event_vector_dict, timeout=0)
  ...
  for option in [1, 2]
    if timeout == 0 
      command = "python3 src/synthesis/z3_event_search.py $(option)"
    else
      if Sys.islinux() 
        command = "gtimeout $(timeout) python3 src/synthesis/z3_event_search.py $(option)"
      else
        command = "timeout $(timeout) python3 src/synthesis/z3_event_search.py $(option)"
      end
    end
    z3_output = try 
...
function z3_event_search_full(run_id, observed_data_dict, event_vector_dict, redundant_events_set, partial=false, timeout=0)
  ...
  for option in options
    shortest_length = 0
    if timeout == 0 
      command = "python3 src/synthesis/z3_event_search_full.py $(option) $(run_id) $(shortest_length)"
    else
      if Sys.islinux() 
        command = "gtimeout $(timeout) python3 src/synthesis/z3_event_search_full.py $(option) $(run_id) $(shortest_length)"
      else
        command = "timeout $(timeout) python3 src/synthesis/z3_event_search_full.py $(option) $(run_id) $(shortest_length)"
      end
    end
    z3_output = try 
                  readchomp(eval(Meta.parse("`$(command)`")))
