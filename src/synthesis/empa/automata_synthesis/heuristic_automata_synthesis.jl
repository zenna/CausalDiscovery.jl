...
function generate_on_clauses_GLOBAL(run_id, matrix, unformatted_matrix, object_decomposition, user_events, global_event_vector_dict, redundant_events_set, grid_size=16, desired_solution_count=1, desired_per_matrix_solution_count=1, interval_painting_param=false, sketch=false, z3_option="full", time_based=false, z3_timeout=0, sketch_timeout=0, co_occurring_param=false, transition_param=false, co_occurring_distinct=1, co_occurring_same=1, co_occurring_threshold=1, transition_distinct=1, transition_same=1, transition_threshold=1, num_transition_decisions=15, pedro=true; state_synthesis_algorithm="heuristic", symmetry=false, stop_times=[])
  ...
  for filtered_matrix_index in 1:length(filtered_matrices)
    ...
    JLD.save("scratch/$(run_id)_intermediate_data.jld", "data", (new_on_clauses, state_based_update_functions_dict, ordered_update_functions_dict, observation_vectors_dict))
        ...
        while problem_contexts != [] && solutions_per_matrix_count < desired_per_matrix_solution_count
          ...
          if state_synthesis_algorithm == "sketch_multi"
            ...
            if length(collect(keys(object_specific_update_functions_dict))) > 0 
              ...
              object_specific_state_solutions_dict_full = object_specific_state_solutions_dict
          ...
          elseif state_synthesis_algorithm == "heuristic"
            ...
            while length(collect(keys(co_occurring_events_dict))) != 0
              ...
              new_on_clauses, state_based_update_functions_dict, _, _, global_event_vector_dict, _ = generate_stateless_on_clauses(run_id, type_displacements, interval_offsets, source_exists_events_dict, update_functions_dict, matrix, filtered_matrix, anonymized_filtered_matrix, global_object_decomposition, user_events, state_update_on_clauses, global_var_dict, global_event_vector_dict, redundant_events_set, z3_option, time_based, z3_timeout, sketch_timeout, symmetry, stop_times=stop_times)          
  ...
  for type_id in type_ids
    ...
    for update_rule in update_functions
      ...
      if update_rule != "" && !is_no_change_rule(update_rule)
        ...
        println(update_rule)
        events, event_is_globals, event_vector_dict, observation_data_dict = generate_event(run_id, type_displacements, interval_offsets, source_exists_events_dict, update_rule, all_update_rules, object_ids[1], object_ids, matrix, filtered_matrix, global_object_decomposition, user_events, state_update_on_clauses, global_var_dict, global_event_vector_dict, grid_size, redundant_events_set, 1, 400, z3_option, time_based, z3_timeout, sketch_timeout, symmetry, stop_times=stop_times)
