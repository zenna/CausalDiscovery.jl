...
function num_false_positives(event_vector, update_function_times, object_trajectory; addObj=false)
  event_times = findall(x -> x == 1, event_vector)
  length([ time for time in event_times if !(time in update_function_times) && (addObj ? true : !isnothing(object_trajectory[time])) ])
end
